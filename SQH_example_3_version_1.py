'''
First version of the implementation of the example 4.4 from page 99 of Pinch, Enid R., Optimal Control and the Calculus of Variations, Oxford University Press, 1995
...
def sqh_algorithm(epsilon):
    '''Implementation of the SQH algorithm with an additional loop for the penalty parameter rho
        ...
        return:
            ...
            l (int), number of iterations of the SQH algorithm until convergence or maximum number of iterations is reached
    '''
    # Initialize variables
    x_start = np.array([0]) # Given initial state
    k_max = 100  # Maximum number of iterations for the inner loop
    l_max = 1000 # Maximum number of iterations for the outer loop
    kappa = 1e-8  # Tolerance for the convergence criterion
    kappa_2 = 1e-3 # Tolerance for the end condition
    k = 0 # Iteration counter for inner loop
    l = 0 # Iteration counter for outer loop
    ...
    cond = True # Parameter for the third while loop, initialize with True
    epsilon_0 = epsilon # Initial value for epsilon (needs to be the starting value for each iteration of the outer loop)
    ...
    while l < l_max and end_cond(x_k) > kappa_2:
        k = 0 # Reset the iteration counter for the inner loop
        tau = kappa + 1 # Reset the parameter for the first while loop
        epsilon = epsilon_0 # Reset epsilon to the initial value
        ...
        while k < k_max and tau > kappa:
            ...
            p_end = np.array([2 * rho * abs(2 - x_k[-1])]) # Initial value for the adjoint state at the end time
            ...
            # Step 6: Update the inner iteration count, control and state
            k += 1
            u_k = u_k_plus_1
            x_k = x_k_plus_1
            print("tau =", tau)

        # Update the outer iteration count and rho
        print("k =", k)
        l += 1
        rho += 2 # Increase the penalty parameter for the objective function
        print("rho =", rho)
        print("end condition:", end_cond(x_k))

    
    return t_grid, x_k, u_k, l # Return the time grid, the optimal state, the optimal control and the number of iterations
...
def main():
    ...
    t_grid, x_k, u_k, l = sqh_algorithm(epsilon=3) # Run the SQH algorithm for the optimal epsilon = 3 (optimal for this example, see experiment_3.py) and get the optimal state and control and the number of iterations
